...
public class TabFragment extends android.support.v4.app.Fragment implements ViewPager.OnPageChangeListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        if (savedInstanceState == null) {
            ...
            int i = tabs1.size();
            if (i == 0) {
                if (mainActivity.storage_count > 1) addTab(new Tab(1, "", ((EntryItem) DataUtils.list.get(1)).getPath(), "/"), 1, "");
                else addTab(new Tab(1, "", "/", "/"), 1, "");
                ...
                if (path != null && path.length() != 0) {
                    if (l == 1) addTab(tabHandler.findTab(1), 1, "");
                    addTab(tabHandler.findTab(l + 1), l + 1, path);
                    if (l == 0) addTab(tabHandler.findTab(2), 2, "");
                } else {
    ...
    public void updatepaths(int pos) {
        if (tabHandler == null) tabHandler = new TabHandler(getActivity(), null, null, 1);
        int i = 1;
        ...
        for (Fragment fragment : fragments) {
            if (fragment.getClass().getName().contains("Main")) {
                ...
                } else tabHandler.addTab(new Tab(i, m.home, m.home, m.home));
    ...
    String parsePathForName(String path, int openmode) {
        ...
        if ("/".equals(path)) return resources.getString(R.string.rootdirectory);
        else if (openmode == 1 && path.startsWith("smb:/")) return (new File(parseSmbPath(path)).getName());
        else if ("/storage/emulated/0".equals(path)) return resources.getString(R.string.storage);
        else if (openmode == 2) return new MainActivityHelper(mainActivity).getIntegralNames(path);
    ...
    public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
        ...
        int color = (int) evaluator.evaluate(position + positionOffset, Color.parseColor(startColor), Color.parseColor(endColor));
            ...
            if (name != null && name.contains("Main")) {
                ...
                if (ma.CURRENT_PATH != null) {
                    try {
                        ...
                        mainActivity.updatePath(ma.CURRENT_PATH, ma.results, ma.openMode, ma.folder_count, ma.file_count);
    ...
    public void addTab(Tab text, int pos, String path) {
        ...
        Bundle b = new Bundle();
        if (path != null && path.length() != 0) b.putString("lastpath", path);
        else b.putString("lastpath", text.getOriginalPath(savepaths));
